...
public class FirstPersonCharacter : MonoBehaviour, IOnCollisionEnterProxy, IOnCollisionStayProxy
{
	...
	private void Update()
	{
		...
		else if (this.allowJump && this.CanJump && (this.Grounded || (Time.time < this.fauxGroundedTimer && !this.blockFauxJump)) && TheForest.Utils.Input.GetButtonDown("Jump") && !LocalPlayer.AnimControl.useRootMotion && !this.crouchHeightBlocked && !LocalPlayer.AnimControl.doSledPushMode && !LocalPlayer.AnimControl.onRope && !this.fsmClimbBool.Value && !this.Diving && !this.Locked && !this.jumpInputBlock && LocalPlayer.Inventory.CurrentView != PlayerInventory.PlayerViews.Inventory)
		{
		...
		if (!this.MovementLocked && (!this.Locked || (this.Grounded && !this.prevGrounded)))
		{
			...
			if (this.Grounded || this.swimming)
			{
				...
				else
				{
					...
					else if (!LocalPlayer.AnimControl.doShellRideMode)
					{
	...
	public void HandleLanded()
	{
		...
		float num = 28f;
		bool flag = false;
		if (LocalPlayer.AnimControl.doShellRideMode && this.prevVelocityXZ.magnitude > 32f)
		{
			flag = true;
		}
		if (this.prevVelocity > num && !flag && this.allowFallDamage && this.jumpingTimer > 0.75f)
		{
			if (!this.jumpLand && !Clock.planecrash)
			{
				...
				this.jumpLand = true;
				float num2 = this.prevVelocity * 1.1f * (this.prevVelocity / 27.5f);
				int damage = (int)num2;
				float num3 = 3.8f;
				if (LocalPlayer.AnimControl.doShellRideMode)
				{
	...
	private Vector3 DetermineVelocityChange()
	{
		...
		if (this.PushingSled || LocalPlayer.AnimControl.doShellRideMode)
		{
			vector.x = 0f;
		}
		if (LocalPlayer.AnimControl.doShellRideMode)
	...
	private void ApplyGroundingForce()
	{
		...
		{
			if (!LocalPlayer.AnimControl.doShellRideMode || !this.buoyancy.InWater)
			{
