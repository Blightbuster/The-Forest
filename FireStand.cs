...
public class FireStand : EntityBehaviour<IFireState>, IEventListener
{
	...
	private void Awake()
	{
		...
		{
			GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(Prefabs.Instance.Constructions._blueprints.Find((BuildingBlueprint bp) => bp._type == BuildingTypes.FireStand)._builtPrefab);
	...
	private void Update()
	{
		...
		if (this.Player != null)
		{
			...
			if (this.Lit && !this.LeafDelay && this.Player.Owns(this._leafItemId, true))
			{
				this.AddLeavesIcon.SetActive(true);
				if (TheForest.Utils.Input.GetButtonDown("Craft") && this.Player.RemoveItem(this._leafItemId, 1, false, true))
				{
