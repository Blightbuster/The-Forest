...
namespace Pathfinding.Serialization
{
	...
	public class TinyJsonSerializer
	{
		...
		private TinyJsonSerializer()
		{
			...
			{
				this.output.AppendFormat("{{ \"x\": {0}, \"y\": {1} }}", ((Vector2)v).x.ToString("R"), ((Vector2)v).y.ToString("R"));
			};
			this.serializers[typeof(Vector3)] = delegate(object v)
			{
				this.output.AppendFormat("{{ \"x\": {0}, \"y\": {1}, \"z\": {2} }}", ((Vector3)v).x.ToString("R"), ((Vector3)v).y.ToString("R"), ((Vector3)v).z.ToString("R"));
			};
