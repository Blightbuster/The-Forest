...
namespace Pathfinding
{
	...
	public class GridGraph : NavGraph, IUpdatableGraph, IRaycastableGraph
	{
		...
		{
			return n1.Walkable && n2.Walkable && (this.maxClimb <= 0f || (float)Math.Abs(n1.position[this.maxClimbAxis] - n2.position[this.maxClimbAxis]) <= this.maxClimb * 1000f);
		}
		...
		public virtual void CalculateConnections(int x, int z, GridNode node)
		{
			...
			if (this.neighbours == NumNeighbours.Four || this.neighbours == NumNeighbours.Eight)
			{
				...
				for (int i = 0; i < 4; i++)
				{
					...
					{
						GridNode n = this.nodes[nodeInGridIndex + this.neighbourOffsets[i]];
				...
				if (this.neighbours == NumNeighbours.Eight)
				{
					if (this.cutCorners)
					{
						for (int j = 0; j < 4; j++)
						{
							if (((num >> j | num >> j + 1 | num >> j + 1 - 4) & 1) != 0)
							{
								...
								{
									GridNode n2 = this.nodes[nodeInGridIndex + this.neighbourOffsets[num5]];
					...
					else
					{
						for (int k = 0; k < 4; k++)
						{
							...
							{
								GridNode n3 = this.nodes[nodeInGridIndex + this.neighbourOffsets[k + 4]];
			...
			else
			{
				...
				for (int l = 0; l < GridGraph.hexagonNeighbourIndices.Length; l++)
				{
					...
					{
						GridNode n4 = this.nodes[nodeInGridIndex + this.neighbourOffsets[num8]];
		...
		public bool Linecast(Vector3 _a, Vector3 _b, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace)
		{
			...
			while (gridNodeBase3.NodeInGridIndex != gridNodeBase2.NodeInGridIndex)
			{
				...
				if (neighbourAlongDirection == null)
				{
					...
					Vector2 vector6 = VectorMath.LineIntersectionPoint(vector5, vector5 + b, vector3, vector4);
