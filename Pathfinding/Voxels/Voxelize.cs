...
namespace Pathfinding.Voxels
{
	...
	public class Voxelize
	{
		...
		public void BuildPolyMesh(VoxelContourSet cset, int nvp, out VoxelMesh mesh)
		{
			...
			Buffer.BlockCopy(array2, 0, array7, 0, num5 * 4);
			Buffer.BlockCopy(array3, 0, array8, 0, num6 * 4);
			mesh.verts = array6;
			mesh.tris = array7;
			mesh.areas = array8;
		}
		...
		public void VoxelizeInput()
		{
			...
			float num4 = Mathf.Cos(Mathf.Atan(Mathf.Tan(this.maxSlope * 0.0174532924f) * (num3 * this.cellSize)));
			...
			{
				num5 = Math.Max(this.inputMeshes[i].vertices.Length, num5);
			}
			Vector3[] array6 = new Vector3[num5];
			Matrix4x4 lhs = Matrix4x4.TRS(-new Vector3(0.5f, 0f, 0.5f), Quaternion.identity, Vector3.one) * Matrix4x4.Scale(new Vector3(num2, num3, num2)) * Matrix4x4.TRS(-min, Quaternion.identity, Vector3.one);
			for (int j = 0; j < this.inputMeshes.Count; j++)
			{
				RasterizationMesh rasterizationMesh = this.inputMeshes[j];
				Matrix4x4 matrix4x = rasterizationMesh.matrix;
				matrix4x = lhs * matrix4x;
				bool flag = VectorMath.ReversesFaceOrientations(matrix4x);
				...
				for (int l = 0; l < num6; l += 3)
				{
					...
					int num7 = (int)Utility.Min(vector.x, vector2.x, vector3.x);
					int num8 = (int)Utility.Min(vector.z, vector2.z, vector3.z);
					int num9 = (int)Math.Ceiling((double)Utility.Max(vector.x, vector2.x, vector3.x));
					int num10 = (int)Math.Ceiling((double)Utility.Max(vector.z, vector2.z, vector3.z));
					num7 = Mathf.Clamp(num7, 0, this.voxelArea.width - 1);
					num9 = Mathf.Clamp(num9, 0, this.voxelArea.width - 1);
					num8 = Mathf.Clamp(num8, 0, this.voxelArea.depth - 1);
					num10 = Mathf.Clamp(num10, 0, this.voxelArea.depth - 1);
					if (num7 < this.voxelArea.width && num8 < this.voxelArea.depth && num9 > 0 && num10 > 0)
					{
						float num11 = Vector3.Dot(Vector3.Cross(vector2 - vector, vector3 - vector).normalized, Vector3.up);
						int area2;
						if (num11 < num4)
						{
							area2 = 0;
						...
						for (int m = num7; m <= num9; m++)
						{
							...
							if (num12 >= 3)
							{
								...
								if (num12 >= 3)
								{
									...
									int num15 = Mathf.Clamp((int)Math.Round((double)num13), 0, this.voxelArea.depth - 1);
									int num16 = Mathf.Clamp((int)Math.Round((double)num14), 0, this.voxelArea.depth - 1);
										...
										if (num18 >= 3)
										{
											...
											if (num18 >= 3)
											{
												...
												for (int num21 = 1; num21 < num18; num21++)
												{
													...
													num19 = Math.Min(num19, val2);
													num20 = Math.Max(num20, val2);
												}
												int num22 = (int)Math.Ceiling((double)num20);
												if (num22 >= 0 && num19 <= (float)num)
												{
													int num23 = Math.Max(0, (int)num19);
													num22 = Math.Max(num23 + 1, num22);
		...
		private void DrawLine(int a, int b, int[] indices, int[] verts, Color col)
		{
			...
			Debug.DrawLine(this.VoxelToWorld(verts[num], verts[num + 1], verts[num + 2]), this.VoxelToWorld(verts[num2], verts[num2 + 1], verts[num2 + 2]), col);
		}
