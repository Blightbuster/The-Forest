...
public class FarShadowCascadeDrawMesh : MonoBehaviour
{
	...
	private void Awake()
	{
		this.farCascadeProp = Shader.PropertyToID("_FarCascade");
		this.billboardShadeFadeFactorsProp = Shader.PropertyToID("_BillboardShadeFadeFactors");
		this.farCascadeMatrixProp = Shader.PropertyToID("_FarCascadeMatrix");
		this.farCascadeBlendValuesProps = Shader.PropertyToID("_FarCascadeBlendValues");
		if (!this.lightCamera)
		{
			this.Init();
		}
		...
		this.mat = new Material(Shader.Find("Hidden/Far Shadow Depth Only"));
	}
	...
	private Matrix4x4[] targetMatrices;
	...
	private int farCascadeProp;

	
	private int billboardShadeFadeFactorsProp;

	
	private int farCascadeMatrixProp;

	
	private int farCascadeBlendValuesProps;
