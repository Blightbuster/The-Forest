...
namespace TheForest.Items.World
{
	...
	public class CaveMapDrawer : MonoBehaviour
	{
		...
		{
			if (!base.enabled && LocalPlayer.Inventory.Owns(this._caveMapItemId, true))
			{
				base.enabled = true;
				if (this._visitedAreas == null)
				{
		...
		{
			if (!this._togglingMaterial && this._renderer.sharedMaterial.Equals(this._caveMap) != LocalPlayer.IsInCaves)
			{
