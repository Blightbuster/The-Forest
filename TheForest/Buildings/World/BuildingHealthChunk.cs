...
namespace TheForest.Buildings.World
{
	...
	public class BuildingHealthChunk : EntityEventListener<IBuildingChunkDestructibleState>
	{
		...
		{
			if (!PlayerPreferences.NoDestruction && this._nextHit < Time.realtimeSinceStartup)
			{
				...
				if (BoltNetwork.isRunning)
				{
					...
					{
						Prefabs.Instance.SpawnHitPS(this._bh._particleType, data._position, Quaternion.LookRotation(base.transform.right));
		...
		{
			if (!PlayerPreferences.NoDestruction && this._hp > 0f && data._damage > 0f)
			{
				this._hp -= data._damage;
				int collapsedLogs = 0;
				Prefabs.Instance.SpawnHitPS(this._bh._particleType, data._position, Quaternion.LookRotation(data._position - base.transform.position));
				if (this._hp <= 0f)
				{
