...
namespace TheForest.Utils
{
	...
	public static class MathEx
	{
		...
		public static bool AreLineSegmentsCrossing(Vector3 pointA1, Vector3 pointA2, Vector3 pointB1, Vector3 pointB2)
		{
			...
			Vector3 point;
			Vector3 point2;
			bool flag = MathEx.ClosestPointsOnTwoLines(out point, out point2, pointA1, vector.normalized, pointB1, vector2.normalized);
			if (flag)
			{
				int num = MathEx.PointOnWhichSideOfLineSegment(pointA1, pointA2, point);
				int num2 = MathEx.PointOnWhichSideOfLineSegment(pointB1, pointB2, point2);
				return num == 0 && num2 == 0;
			}
			return false;
		}
		...
		{
			for (int i = 1; i < multipointList.Count; i++)
			{
				if (MathEx.AreLineSegmentsCrossing(pointA1, pointA2, multipointList[i - 1], multipointList[i]))
				{
					return true;
				}
			}
		...
		public static Vector3 TryAngleSnap(Vector3 point, Vector3 center, Vector3 edgePoint, float snapAngle, float snapRange, bool flattenYAxis = true)
		{
			...
			Vector3 vector2 = edgePoint - center;
			if (flattenYAxis)
			{
				vector.y = 0f;
				vector2.y = 0f;
			...
			if (Vector3.Cross(vector2, vector).y < 0f)
			{
				num *= -1f;
			}
			bool flag = Mathf.Abs(num - (float)Mathf.RoundToInt(num / snapAngle) * snapAngle) < snapRange;
			if (flag)
			{
				float angle = (float)Mathf.RoundToInt(num / snapAngle) * snapAngle;
				Vector3 b = Vector3.Project(vector, vector2.RotateY(angle));
				point = center + b;
			}
			return point;
		}
		...
		public static float ClosestDistanceToPolygon(Vector3 point, List<Vector3> polygon)
		{
			...
			for (int i = 1; i < polygon.Count; i++)
			{
				Vector3 a = MathEx.ProjectPointOnLineSegment(polygon[i], polygon[i - 1], point);
		...
		public static List<Vector3> CalculateMeshPerimeter(Mesh mesh)
		{
			...
			int num = -1;
			foreach (KeyValuePair<MathEx.Edge, int> keyValuePair in edgeTable.faceCounts)
			{
				if (keyValuePair.Value == 1)
				{
					vertexTable.AddEdge(keyValuePair.Key);
					num = keyValuePair.Key.v1;
				}
			}
		...
		private class ContourState
		{
			...
			private unsafe List<Vector2> ExtractNextContour()
			{
				...
				while (this.CalculateNextCell(lastColumn, lastRow, num3, num4, out num5, out num6, out index))
				{
					...
					num3 = num5;
					num4 = num6;
					byte* ptr = ref this.visited[num3, num4];
					*ptr += 1;
		...
		private class ErasureSpan
		{
			...
			public float maximumError { get; private set; }
			...
			private void CalculateMaximumError(IList<Vector2> polygon)
			{
				...
				for (int num = (this.start + 1) % polygon.Count; num != this.end; num = (num + 1) % polygon.Count)
				{
					...
					{
						this.maximumError = float.MaxValue;
						break;
					}
		...
		private class MergeSpan
		{
			...
			public int mergeStart { get; private set; }

			
			
			
			public int mergeEnd { get; private set; }
			...
			public Vector2 mergePoint { get; private set; }
		...
		public class Easing
		{
			...
			{
				time /= duration;
				return range * time * time * time * time + offset;
			}
		}
	}
}
