	...
	public class PlayerPushSledAction : MonoBehaviour
	{
		...
		private void enterPushSled(Transform trn)
		{
			...
			if (!this.fsmPushSledBool.Value)
			{
				...
				this.currentSledRoot.gameObject.GetComponent<Rigidbody>().isKinematic = true;
				...
				LocalPlayer.CamRotator.rotationRange = new Vector2(100f, 110f);
				...
				this._player.HideAllEquiped(false, false);
		...
		public void forceExitSled()
		{
			if (this.currentSled)
			{
				...
				this.currentSled.SendMessage("resetTrigger", SendMessageOptions.DontRequireReceiver);
				this.currentSled.SendMessage("disableSled", true, SendMessageOptions.DontRequireReceiver);
				this.exitPushSled();
				return;
			}
		}
		...
		private void exitPushSled()
		{
			if (this.doingAction)
			{
				...
				LocalPlayer.FpCharacter.StartCoroutine("DisconnectedSledRoutine");
				float y = LocalPlayer.Transform.localEulerAngles.y;
				this._player.PM.SendEvent("toExitPushSled");
				this._player.ShowAllEquiped(true);
				...
				this.currentSledRoot.gameObject.layer = 28;
				...
				rigidbody.mass = 110f;
				if (this.currentSledRoot)
				{
					this.currentSledRoot.SendMessage("fixFlyingSledRoutine", SendMessageOptions.DontRequireReceiver);
				}
				...
				LocalPlayer.AnimControl.fullyAttachedToSled = false;
				this.currentSled.SendMessage("Interraction", true, SendMessageOptions.DontRequireReceiver);
				if (Grabber.Filter == this.currentSled.gameObject)
				{
					Grabber.Filter = null;
				}
				this.currentSled = null;
		...
		private void connectRigidBody()
		{
			if (LocalPlayer.FpCharacter.PushingSled)
			{
				...
				LocalPlayer.AnimControl.fullyAttachedToSled = true;
			}
			base.Invoke("fixSledPosition", 0.1f);
		}

		
		private void fixSledPosition()
		{
			if (this.doingAction && this.currentSledRoot)
			{
				this.currentSledRoot.transform.localPosition = new Vector3(-1.32f, -1.3f, 3.155f);
				this.currentSledRoot.transform.localEulerAngles = new Vector3(-26.5f, -0.29f, -0.51f);
			}
