...
public class SetupAdvancedFoliageShader : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		Shader.SetGlobalVector(this.afsSpecFadeProp, new Vector4(this.AfsSpecFade.x, this.AfsSpecFade.y, 1f, 1f));
	}
	...
	private void getShaderProps()
	{
		this.afsDirectSunDirProp = Shader.PropertyToID("_AfsDirectSunDir");
		this.afsDirectSunColProp = Shader.PropertyToID("_AfsDirectSunCol");
		this.afsSpecFadeProp = Shader.PropertyToID("_AfsSpecFade");
		this.windProp = Shader.PropertyToID("_Wind");
		this.windStrengthProp = Shader.PropertyToID("_WindStrengthTrees");
		this.windFrequencyProp = Shader.PropertyToID("_WindFrequency");
		this.afsTimeFrequencyProp = Shader.PropertyToID("_AfsTimeFrequency");
		this.afsWaveSizeProp = Shader.PropertyToID("_AfsWaveSize");
		this.afsWindJitterScaleProp = Shader.PropertyToID("_AfsWindJitterScale");
		this.afsGrassWindProp = Shader.PropertyToID("_AfsGrassWind");
		this.afsWaveAndDistanceProp = Shader.PropertyToID("_AfsWaveAndDistance");
		this.afsWindMuliplierProp = Shader.PropertyToID("_AFSWindMuliplier");
		this.afsRainamountProp = Shader.PropertyToID("_AfsRainamount");
		this.afsWavingTintProp = Shader.PropertyToID("_AfsWavingTint");
	}
	...
	private void afsLightingSettings()
	{
		if (this.DirectionalLightReference != null)
		{
			...
			{
				Shader.SetGlobalVector(this.afsDirectSunDirProp, new Vector4(0f, 0f, 0f, 0f));
			}
			else
			{
				...
				Shader.SetGlobalVector(this.afsDirectSunDirProp, new Vector4(this.DirectlightDir.x, this.DirectlightDir.y, this.DirectlightDir.z, this.SunLuminance));
				Shader.SetGlobalVector(this.afsDirectSunColProp, this.SunLightCol);
			}
		}
		else
		{
			Shader.SetGlobalVector(this.afsDirectSunDirProp, new Vector4(0f, 0f, 0f, 0f));
			Shader.SetGlobalVector(this.afsDirectSunColProp, new Vector3(0f, 0f, 0f));
		}
	}
	...
	private void afsUpdateWind()
	{
		...
		Shader.SetGlobalVector(this.windProp, this.TempWind);
		Shader.SetGlobalVector(this.windStrengthProp, new Vector2(this.TempWind.w * this.WindMuliplierForTreeShaderPrimary, this.TempWind.w * this.WindMuliplierForTreeShaderSecondary));
		Shader.SetGlobalFloat(this.windFrequencyProp, this.WindFrequency);
		this.domainTime_Wind += this.temp_WindFrequency * Time.deltaTime * 2f;
		this.domainTime_2ndBending += Time.deltaTime;
		Shader.SetGlobalVector(this.afsTimeFrequencyProp, new Vector4(this.domainTime_Wind, this.domainTime_2ndBending, 0.375f * (1f + this.temp_WindFrequency * this.LeafTurbulenceFoliageShader), 0.193f * (1f + this.temp_WindFrequency * this.LeafTurbulenceFoliageShader)));
		...
		Shader.SetGlobalFloat(this.afsWaveSizeProp, 0.5f / this.WaveSizeFoliageShader);
		float num = this.WindJitterScaleForGrassshaderCurve.Evaluate(this.TempWind.w);
		Shader.SetGlobalFloat(this.afsWindJitterScaleProp, this.WindJitterScaleForGrassshader * (10f * num));
		Shader.SetGlobalVector(this.afsGrassWindProp, this.TempWind * this.WindMultiplierForGrassshader);
		...
		Shader.SetGlobalVector(this.afsWaveAndDistanceProp, new Vector4(this.GrassWindSpeed, this.WaveSizeForGrassshader, this.TempWind.w, this.DetailDistanceForGrassShader * this.DetailDistanceForGrassShader));
		Shader.SetGlobalVector(this.afsWindMuliplierProp, this.WindMuliplierForTreeShader);
	}

	
	private void afsUpdateRain()
	{
		Shader.SetGlobalFloat(this.afsRainamountProp, this.RainAmount);
	}
	...
	{
		Shader.SetGlobalColor(this.afsWavingTintProp, this.GrassWavingTint);
	}
	...
	private int afsDirectSunDirProp;

	
	private int afsDirectSunColProp;

	
	private int afsSpecFadeProp;

	
	private int windProp;

	
	private int windStrengthProp;

	
	private int windFrequencyProp;

	
	private int afsTimeFrequencyProp;

	
	private int afsWaveSizeProp;

	
	private int afsWindJitterScaleProp;

	
	private int afsGrassWindProp;

	
	private int afsWaveAndDistanceProp;

	
	private int afsWindMuliplierProp;

	
	private int afsRainamountProp;

	
	private int afsWavingTintProp;
}
