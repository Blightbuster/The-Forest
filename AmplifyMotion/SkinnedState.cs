	...
	internal class SkinnedState : MotionState
	{
		...
		internal override void Initialize()
		{
			if (!this.m_renderer.sharedMesh.isReadable)
			{
				...
				{
					Debug.LogWarning("[AmplifyMotion] Read/Write Import Setting disabled in object " + this.m_obj.name + ". Skipping.");
			...
			if (!this.m_useFallback)
			{
				...
				this.m_bones = new Matrix4x4[this.m_boneCount];
		...
		private bool InitializeGPUSkinDeform()
		{
			...
			try
			{
				this.m_gpuBoneTexWidth = Mathf.NextPowerOfTwo(this.m_boneCount);
				this.m_gpuBoneTexHeight = 4;
				this.m_gpuVertexTexWidth = Mathf.NextPowerOfTwo(Mathf.CeilToInt(Mathf.Sqrt((float)this.m_vertexCount)));
				this.m_gpuVertexTexHeight = Mathf.NextPowerOfTwo(Mathf.CeilToInt((float)this.m_vertexCount / (float)this.m_gpuVertexTexWidth));
