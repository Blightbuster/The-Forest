...
public static class FMODNetworkedAudio
{
	...
	public static void PlayOneShot(string eventName, Vector3 position, bool local, float maxDistance)
	{
		if (BoltNetwork.isRunning)
		{
			...
			{
				if ((boltEntity.transform.position - position).sqrMagnitude < maxDistance * maxDistance)
				{
					FMODNetworkedAudio.SendEvent(eventName, position, boltEntity.source);
				}
			}
		}
		if (local)
	...
	public static void PlayOneShotParameter(string eventName, Vector3 position, int index, float value, bool local, float maxDistance)
	{
		if (BoltNetwork.isRunning)
		{
			foreach (BoltEntity boltEntity in Scene.SceneTracker.allPlayerEntities)
			{
				if ((boltEntity.transform.position - position).sqrMagnitude < maxDistance * maxDistance)
				{
					FMODNetworkedAudio.SendEvent(eventName, position, boltEntity.source, index, value);
				}
			}
		}
		if (FMOD_StudioSystem.instance && local)
		{
