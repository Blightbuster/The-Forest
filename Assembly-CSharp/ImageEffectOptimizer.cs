...
public class ImageEffectOptimizer : MonoBehaviour
{
	private void Awake()
	{
		...
		if (this.ppb && this.ppb.profile)
		{
			this.ppb.profile = InstanceManager.GetSharedInstance<PostProcessingProfile>(this.ppb.profile);
			this.postProcessingProfile = this.ppb.profile;
		}
		this.MyCamera = base.gameObject.GetComponent<Camera>();
		this.MyCamera.eventMask = 0;
		this.vClouds = base.GetComponent<RaymarchedClouds>();
		...
		this.depthBuffer = base.GetComponent<DepthBufferGrabCommand>();
		this.aa = base.GetComponent<AntiAliasing>();
	}
	...
	private void Update()
	{
		...
		if (this.vClouds)
		{
			if (LocalPlayer.CurrentView == PlayerInventory.PlayerViews.PlaneCrash || ForestVR.Enabled)
			{
				this.vClouds.enabled = false;
			}
			else if (TheForestQualitySettings.UserSettings.volumetricClouds == TheForestQualitySettings.VolumetricClouds.On)
			{
				this.vClouds.enabled = true;
			}
			else
			{
				this.vClouds.enabled = false;
		...
		if (ForestVR.Enabled && this.aa)
		{
			this.aa.enabled = (PlayerPreferences.VRAntiAliasing != PlayerPreferences.VRAntiAliasingTypes.OFF);
			if (this.aa.enabled)
			{
				this.aa.method = ((PlayerPreferences.VRAntiAliasing != PlayerPreferences.VRAntiAliasingTypes.SMAA) ? 1 : 0);
			}
		}
		if (this.postProcessingProfile)
		{
		...
		if (Application.isPlaying)
		{
			...
			if (Sunshine.Instance)
			{
				Sunshine.Instance.enabled = (ImageEffectOptimizer.AllowSunshine && (!ForestVR.Enabled || LocalPlayer.CurrentView != PlayerInventory.PlayerViews.Pause));
		...
		this.MyCamera.useOcclusionCulling = ImageEffectOptimizer.AllowCulling;
		this.depthBuffer.enabled = ImageEffectOptimizer.AllowDepthBuffer;
		this.ppb.enabled = (ImageEffectOptimizer.AllowPostProcessing && (!ForestVR.Enabled || !LocalPlayer.IsInPauseMenu));
		if (ImageEffectOptimizer.ForceRenderingPath && this.MyCamera)
		{
			this.MyCamera.renderingPath = ImageEffectOptimizer.ForcedRenderingPath;
		}
	}
	...
	private RaymarchedClouds vClouds;
	...
	private DepthBufferGrabCommand depthBuffer;
	...
	internal static bool AllowSunshine = true;

	internal static bool AllowCulling = true;

	internal static bool AllowDepthBuffer = true;

	internal static bool AllowPostProcessing = true;

	internal static bool ForceRenderingPath;

	internal static RenderingPath ForcedRenderingPath = RenderingPath.UsePlayerSettings;
}
