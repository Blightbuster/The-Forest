...
public class TriggerCutScene : MonoBehaviour
{
	...
	public IEnumerator beginPlaneCrash()
	{
		...
		if (ForestVR.Enabled)
		{
			foreach (GameObject gameObject in LocalPlayer.vrAdapter.tempHands)
			{
				gameObject.SetActive(true);
			}
			LocalPlayer.vrAdapter.VRPlayerHands.gameObject.SetActive(false);
			LocalPlayer.vrAdapter.VRWatchParentGo.SetActive(false);
		}
		if (!this.crawl)
		{
			this.crawl = this.PlaneReal.GetComponentInChildren<setupPlayerCrawl>();
		...
		LocalPlayer.MainCam.SendMessage("GuiOff", SendMessageOptions.DontRequireReceiver);
	...
	private void Update()
	{
		...
		{
			if (!Scene.HudGui.PauseMenu.activeSelf)
			{
				if (TheForest.Utils.Input.GetButtonDown("Jump") || TheForest.Utils.Input.GetButtonDown("Skip") || TriggerCutScene.FastStart)
				{
			...
			else if (TheForest.Utils.Input.GetButtonDown("Esc"))
			{
	...
	private IEnumerator startPlayerInPlane()
	{
		...
		LocalPlayer.MainCam.SendMessage("GuiOn", SendMessageOptions.RequireReceiver);
		LocalPlayer.AnimControl.introCutScene = true;
		...
		if (ForestVR.Enabled)
		{
			...
			LocalPlayer.vrAdapter.VRWatchParentGo.SetActive(true);
			...
			LocalPlayer.vrPlayerControl.VROffsetTransform.position = LocalPlayer.vrAdapter.behindCamPos.position;
		}
		if (!TriggerCutScene.FastStart)
		{
		...
		while (state.IsTag("getup"))
		{
			...
			LocalPlayer.Animator.Play("Base Layer.idle", 0, 0f);
	...
	public Transform ClientSeatPosVR;
	...
	public Transform playerAislePositionVR;

	public Transform ClientAislePositionVR;
