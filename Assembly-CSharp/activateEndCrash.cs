...
public class activateEndCrash : MonoBehaviour
{
	...
	private void GrabExit()
	{
		...
		base.enabled = false;
		if (LocalPlayer.Inventory.CurrentView == PlayerInventory.PlayerViews.EndCrash || LocalPlayer.Inventory.CurrentView == PlayerInventory.PlayerViews.EndDeactivate)
		{
	...
	private void Update()
	{
		if (LocalPlayer.Inventory.CurrentView == PlayerInventory.PlayerViews.World && TheForest.Utils.Input.GetButtonAfterDelay("Take", this.delay, false))
		{
			...
			else
			{
				...
				if (this._altEnding)
				{
					LocalPlayer.Inventory.CurrentView = PlayerInventory.PlayerViews.EndDeactivate;
				}
				else
				{
		...
		if (LocalPlayer.Inventory.CurrentView == PlayerInventory.PlayerViews.EndCrash || LocalPlayer.Inventory.CurrentView == PlayerInventory.PlayerViews.EndDeactivate)
		{
			...
			else
			{
				...
				{
					if (Scene.SceneTracker.allPlayerEntities[i].GetState<IPlayerState>().CurrentView == (int)LocalPlayer.CurrentView)
					{
						num++;
					}
				}
