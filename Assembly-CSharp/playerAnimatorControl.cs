...
public class playerAnimatorControl : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (ForestVR.Enabled)
		{
			this.animator.SetFloatReflected("VRBlend", 1f);
			LocalPlayer.ScriptSetup.targetInfo.VREnabled = true;
		}
		else
		{
			this.animator.SetFloatReflected("VRBlend", 0f);
			LocalPlayer.ScriptSetup.targetInfo.VREnabled = false;
		}
		...
		if (this.flyingGlider)
		{
			this.HandleGliderControlUpdate();
		}
		...
		if (!LocalPlayer.Inventory.IsRightHandEmpty() || this.onRockThrower || LocalPlayer.Animator.GetBool("lookAtPhoto") || LocalPlayer.Animator.GetBool("zipLineAttach") || LocalPlayer.Animator.GetBool("logHeld") || this.coldOffsetBool || LocalPlayer.Animator.GetBool("hangGliderHeld"))
		{
		...
		if (ForestVR.Enabled && !LocalPlayer.vrPlayerControl._startedMoving)
		{
			vector = this.modLocalVel;
		}
		...
		if (ForestVR.Enabled && !LocalPlayer.vrPlayerControl._startedMoving)
		{
			this.tempVelocity = this.modLocalVel;
		}
		...
		if (this.carry && (((FirstPersonCharacter.GetDropInput() || (ForestVR.Enabled && TheForest.Utils.Input.GetButtonDown("Fire1"))) && !this.wallSetup.atWallCheck && !this.holdingGirl) || this.swimming))
		{
			this.DropBody();
		}
		if (this.waterBlock)
		...
		else if (this.doShellRideMode || LocalPlayer.Inventory.HasInSlot(Item.EquipmentSlot.RightHand, this._shellId) || LocalPlayer.FpCharacter.crouch || LocalPlayer.Inventory.HasInSlot(Item.EquipmentSlot.RightHand, this._mapId) || this.animator.GetBool("crossbowHeld"))
		{
			this.animator.SetFloatReflected("inWaterBlend", -10f, 1f, Time.deltaTime * 3f);
		}
	...
	{
		if ((LocalPlayer.Inventory.HasInSlot(Item.EquipmentSlot.RightHand, this._flintLockId) || LocalPlayer.Inventory.HasInSlot(Item.EquipmentSlot.RightHand, this._flaregunId) || LocalPlayer.Inventory.HasInSlot(Item.EquipmentSlot.RightHand, this._crossbowId)) && LocalPlayer.Inventory.CurrentView != PlayerInventory.PlayerViews.Inventory)
		{
			Quaternion quaternion = this.setup.rightArm.rotation;
			Quaternion quaternion2 = this.setup.leftArm.rotation;
			Quaternion quaternion3 = this.setup.rightHandWrist.rotation;
			Quaternion quaternion4 = this.setup.leftHandWrist.rotation;
			if ((this.nextLayerState1.shortNameHash == this.reloadFlintlockHash || this.nextLayerState1.shortNameHash == this.reloadFlareGunlockHash || this.nextLayerState1.shortNameHash == this.crossbowReloadHash) && !LocalPlayer.Inventory.IsLeftHandEmpty())
			{
				LocalPlayer.Inventory.MemorizeItem(Item.EquipmentSlot.LeftHand);
				LocalPlayer.Inventory.StashLeftHand();
			}
			if (this.currLayerState1.shortNameHash == this.reloadFlintlockHash || this.currLayerState1.shortNameHash == this.reloadFlareGunlockHash || this.currLayerState1.shortNameHash == this.crossbowReloadHash)
			{
			...
			if (!LocalPlayer.Inventory.HasInSlot(Item.EquipmentSlot.RightHand, this._crossbowId))
			{
				if (!this.fovAimMode)
				{
				...
				quaternion = Quaternion.AngleAxis(this.normCamX * this.offsetFlintlockArmsMult * this.clampArmsVal * this.animator.GetLayerWeight(4), LocalPlayer.Transform.right) * quaternion;
				...
				quaternion3 = Quaternion.AngleAxis((this.normCamX + this.wristAimOffset) * this.offsetFlintlockHandsMult * this.clampArmsVal * this.animator.GetLayerWeight(4), LocalPlayer.Transform.right) * quaternion3;
				if (!this.animator.GetBool("onHand"))
				{
					this.setup.leftArm.rotation = quaternion2;
					this.setup.leftHandWrist.rotation = quaternion4;
				}
				this.setup.rightArm.rotation = quaternion;
				this.setup.rightHandWrist.rotation = quaternion3;
			}
		}
	...
	private void FixedUpdate()
	{
		...
		if (ForestVR.Enabled && !LocalPlayer.vrPlayerControl._startedMoving)
		{
			this.modLocalVel = this.tr.InverseTransformDirection((this.rootTr.position - this.lastPlayerPos) * 100f);
			this.lastPlayerPos = this.rootTr.position;
		}
		else
		{
			this.lastPlayerPos = this.rootTr.position;
	...
	private void enableBirdOnHand()
	{
		...
		Transform transform = this.setup.smallBirdGo.transform;
		if (ForestVR.Enabled)
		{
			transform = this.setup.smallBirdGoVr.transform;
		}
		this.bird = UnityEngine.Object.Instantiate<GameObject>((GameObject)Resources.Load("CutScene/smallBird_ANIM_landOnFinger_prefab"), transform.position, transform.rotation);
		this.bird.transform.parent = transform;
		...
		component.SetBool((!ForestVR.Enabled) ? "toHand" : "toHandVR", true);
	...
	{
		this.hVel = Mathf.SmoothDamp(this.hVel, TheForest.Utils.Input.GetAxis("Horizontal") * 60f, ref this.curVel, 0.2f);
		this.vVel = Mathf.SmoothDamp(this.vVel, TheForest.Utils.Input.GetAxis("Vertical") * 80f, ref this.curVelY, 0.2f);
		if (Mathf.Abs(this.hVel) > 0.0001f)
		{
			this.rootTr.RotateAround(this.rootTr.position, Vector3.up, this.hVel * Time.deltaTime);
		}
		if (Mathf.Abs(this.vVel) > 0.0001f)
		{
			this.rootTr.RotateAround(this.rootTr.position, this.rootTr.right, this.vVel * Time.deltaTime);
		}
		this.shellBlendVal = Mathf.SmoothDamp(this.hVel, this.shellBlendVal, ref this.shellBlendRef, 0.025f);
	...
	private void HandleGliderControl()
	{
		...
		Vector3 a = base.transform.forward.normalized * this.THRUST_FORCE_MULT;
		Vector3 normalized = velocity.normalized;
		if (normalized.y > 0.33f)
		{
			normalized.y = this.rootTr.forward.normalized.y;
		}
		float num = -base.transform.forward.y;
		num = Mathf.Clamp(num * 2f, -1f, 1f);
		this.smoothForwarDir = Mathf.Lerp(this.smoothForwarDir, num, Time.deltaTime * 1.5f);
		this.debugForwardDir = this.smoothForwarDir;
		float d = Mathf.Clamp(this.smoothForwarDir, 0.05f, 1f);
		a *= d;
		float num2 = Vector3.AngleBetween(normalized, this.rootTr.forward.normalized);
		Vector3 b = -normalized * (num2 * this.OVERSHOOT_SLOWDOWN_MULT) * d;
		a = a * (1f + num2 * this.OVERSHOOT_MULT) + b;
		Vector3 normalized2 = this.rootTr.forward.normalized;
		normalized2.y = 0f;
		normalized2.x = velocity.normalized.x;
		normalized2.z = velocity.normalized.z;
		float num3 = this.smoothForwarDir;
		if (num3 < -0.3f)
		{
			num3 *= -1f;
		}
		Vector3 vector = velocity / 15f;
		vector.y = 0f;
		float num4 = (1f - num3) * vector.magnitude;
		num4 = Mathf.Clamp(num4, 0f, 1f);
		num4 *= num4;
		this.debugForwardMag = num4;
		Vector3 b2 = Vector3.up * this.rb.mass * -Physics.gravity.y * num4 * this.LIFT_FORCE_MULT * Mathf.Clamp(-velocity.y / 1f, 1f, 300f);
		this.debugLift = b2.magnitude;
		this.debugForwardThrust = a.magnitude;
		Vector3 b3 = Vector3.down * this.rb.mass * -Physics.gravity.y * (1f - num4) * this.DOWNFORCE_MULT;
		this.debugDownForce = b3.magnitude;
		this.rb.AddForce(a + b2 + b3, ForceMode.Force);
		if (this.rb.velocity.magnitude > this.MAX_GLIDER_VELOCITY)
		{
			Vector3 velocity2 = this.rb.velocity.normalized * this.MAX_GLIDER_VELOCITY;
			this.rb.velocity = velocity2;
		}
	}
	...
	public int climbOutHash = Animator.StringToHash("climbOut");
	...
	public int getupHash = Animator.StringToHash("getup");

	public int crossbowReloadHash = Animator.StringToHash("crossbowReload");
	...
	public float LIFT_FORCE_MULT = 1.5f;

	public float THRUST_FORCE_MULT = 30f;

	public float OVERSHOOT_SLOWDOWN_MULT = 50f;

	public float OVERSHOOT_MULT = 10f;

	public float DOWNFORCE_MULT = 2f;

	public float MAX_GLIDER_VELOCITY = 30f;
