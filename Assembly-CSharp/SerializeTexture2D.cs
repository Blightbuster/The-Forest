...
public class SerializeTexture2D : SerializerExtensionBase<Texture2D>
{
	...
	public override object Load(object[] data, object instance)
	{
		...
		if (data.Length == 10)
		{
			texture2D = new Texture2D(1, 1, (TextureFormat)data[3], (int)data[5] > 0);
			texture2D.LoadImage((byte[])data[9]);
			texture2D.anisoLevel = (int)data[1];
			texture2D.filterMode = (FilterMode)data[2];
			texture2D.mipMapBias = (float)data[4];
			texture2D.name = (string)data[6];
			texture2D.wrapMode = (TextureWrapMode)data[8];
			texture2D.Apply();
			return texture2D;
		}
		texture2D = new Texture2D((int)data[9], (int)data[4], (TextureFormat)data[3], (int)data[6] > 0);
		texture2D.anisoLevel = (int)data[1];
		texture2D.filterMode = (FilterMode)data[2];
		texture2D.mipMapBias = (float)data[5];
		texture2D.name = (string)data[7];
		texture2D.wrapMode = (TextureWrapMode)data[10];
