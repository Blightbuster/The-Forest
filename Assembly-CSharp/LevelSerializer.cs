...
public static class LevelSerializer
{
	...
	{
		if (LevelSerializer._allPrefabs == null || LevelSerializer._allPrefabs.Count == 0 || (SaveGameManager.Instance && SaveGameManager.Instance != null && SaveGameManager.Instance.requiredObjects.Length > LevelSerializer._allPrefabs.Count))
		{
			if (SaveGameManager.Instance && SaveGameManager.Instance.requiredObjects != null)
			{
				LevelSerializer._allPrefabs = LevelSerializer.GetFilteredRequiredObjects();
			}
			else
			{
				LevelSerializer._allPrefabs = LevelSerializer.GetFilteredResourcesPrefabs();
			}
		}
	}

	private static Dictionary<string, GameObject> GetFilteredResourcesPrefabs()
	{
		return Resources.FindObjectsOfTypeAll(typeof(GameObject)).Cast<GameObject>().Where(delegate(GameObject go)
		{
	...
	private static Dictionary<string, GameObject> GetFilteredRequiredObjects()
	{
		Dictionary<string, GameObject> dictionary = new Dictionary<string, GameObject>();
		foreach (UnityEngine.Object @object in SaveGameManager.Instance.requiredObjects)
		{
			GameObject gameObject = @object as GameObject;
			if (!(gameObject == null))
			{
				PrefabIdentifier component = gameObject.GetComponent<PrefabIdentifier>();
				if (component == null)
				{
					Debug.LogError(string.Format("{0} doesn't have PrefabIdentifier!", gameObject));
				}
				else
				{
					string classId = component.ClassId;
					if (dictionary.ContainsKey(classId))
					{
						Debug.LogError(string.Format("{0} & {1} \"{2}\" duplicated prefab ClassId!", gameObject.name, dictionary[classId].name, classId));
					}
					else
					{
						dictionary.Add(classId, gameObject);
					}
				}
			}
		}
	...
	internal static Dictionary<string, GameObject> AllPrefabs
	{
		get
		{
			...
			{
				LevelSerializer._allPrefabs = (from p in LevelSerializer._allPrefabs
				where p.Value
			...
			return LevelSerializer._allPrefabs;
		}
		set
		{
			LevelSerializer._allPrefabs = value;
		}
	}
	...
	private static Dictionary<string, GameObject> _allPrefabs = new Dictionary<string, GameObject>();
