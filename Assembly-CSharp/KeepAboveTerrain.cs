...
public class KeepAboveTerrain : MonoBehaviour
{
	...
	public Vector3 AimingCastOrigin
	{
		get
		{
			return (!ForestVR.Enabled) ? LocalPlayer.MainCamTr.position : LocalPlayer.InventoryMouseEventsVR.RayOverride.position;
		}
	}

	public Vector3 AimingCastDirection
	{
		get
		{
			return (!ForestVR.Enabled) ? LocalPlayer.MainCamTr.forward : LocalPlayer.InventoryMouseEventsVR.RayOverride.forward;
		}
	}
	...
	private void Update()
	{
		...
		if (!this.Locked && LocalPlayer.Inventory.CurrentView == PlayerInventory.PlayerViews.World)
		{
			...
			bool flag = false;
			Vector3 point;
			if (MathEx.ClosestPointsOnTwoLines(out this.lookingAtPointCam, out point, this.AimingCastOrigin, this.AimingCastDirection, vector, Vector3.up))
			{
				float num2 = Vector3.Distance(this.AimingCastOrigin, this.lookingAtPointCam) * 0.96f;
				if (ForestVR.Enabled)
				{
					point = this.lookingAtPointCam;
				}
				...
				Vector3 vector7;
				if (this.IgnoreLookAtCollision || !Physics.Raycast(this.AimingCastOrigin, this.AimingCastDirection, out this.hit, num2, this.GetValidLayers(this.FloorLayersFinal.value)))
				{
					vector7 = this.AimingCastOrigin + this.AimingCastDirection * num2;
					this.LastHit = null;
					if (this.Airborne)
					{
						vector3.y = point.y;
				...
				if (ForestVR.Enabled)
				{
					Vector3 b2 = vector7 - base.transform.position;
					vector3 += b2;
					vector4 += b2;
					vector5 += b2;
					vector6 += b2;
				...
				{
					Vector3 b3 = new Vector3(1f, 0f, 1f);
					float num3 = Vector3.Distance(Vector3.Scale(vector7, b3), Vector3.Scale(LocalPlayer.Transform.position, b3));
					float num4 = Vector3.Distance(Vector3.Scale(base.transform.position, b3), Vector3.Scale(LocalPlayer.Transform.position, b3));
					if (!Mathf.Approximately(num3, num4))
					{
						Vector3 b4 = LocalPlayer.Transform.forward * (num3 - num4);
			...
			position.y = point.y + 1f;
			float num6;
			if (Physics.SphereCast(position, radius, Vector3.down, out this.hit, maxDistance, this.GetValidLayers(this.FloorLayersFinal.value)))
			{
			...
			Vector3 a = Vector3.Cross(vector4 - vector3, vector5 - vector3);
			Vector3 b5 = Vector3.Cross(vector5 - vector6, vector4 - vector6);
			this.curNormal = Vector3.Normalize((a + b5) / 2f);
