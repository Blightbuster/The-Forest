...
public class mutantDayCycle : MonoBehaviour
{
	...
	public void initWakeUp()
	{
		if (this.setup.spawnGo)
		{
			...
			if (!this.setup.ai.creepy && !this.setup.ai.creepy_male && !this.setup.ai.creepy_baby && !this.setup.ai.creepy_fat)
			{
				...
				if (!this.setup.typeSetup.inCave && !this.setup.pmSleep.FsmVariables.GetFsmBool("inCaveBool").Value)
				{
	...
	public void disableSleepOverride()
	{
	...
	private void setDayConditions()
	{
		...
		if (!this.creepy)
		{
			if (this.setup.typeSetup.inCave && !this.dayBlocker && !this.sleepBlocker)
			{
				this.sleep = true;
		...
		if (this.day == 0 && this.countBlock < 1)
		{
			...
			else
			{
				...
				{
					Scene.SceneTracker.maxAttackers = 3;
				}
				this.aggression = 1;
				this.fear = 4;
		...
		else if (this.day >= 4 && this.countBlock < this.day + 1)
		{
			this.countBlock++;
			if (Clock.Dark)
			{
