...
public class spawnMutants : MonoBehaviour, IMutantSpawner
{
	...
	private IEnumerator doSpawn()
	{
		...
		base.StartCoroutine(this.spawnGirl());
	...
	private IEnumerator spawnRegularMale()
	{
		...
		while (count < this.amount_male)
		{
			...
			{
				mutantAnimatorControl componentInChildren = this.clone.GetComponentInChildren<mutantAnimatorControl>();
				if (componentInChildren)
				{
					componentInChildren.stopMovement = true;
				}
			}
			count++;
		}
	...
	private IEnumerator spawnGirl()
	{
		int count = 0;
		bool doName = false;
		while (count < this.amount_girl)
		{
			...
			Transform transform = UnityEngine.Object.Instantiate<Transform>(this.girl.transform, vector, rotation);
			if (!doName)
			{
				transform.gameObject.name = transform.gameObject.name + this.nameCount;
				doName = true;
			...
			base.StartCoroutine(this.fixMutantPosition(transform, vector));
			transform.SendMessage("setInstantSpawn", this.instantSpawn);
			transform.SendMessage("setGirlMutant", SendMessageOptions.DontRequireReceiver);
			transform.SendMessage("addToSpawn", base.gameObject);
			count++;
		}
		yield return null;
