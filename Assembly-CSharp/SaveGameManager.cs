...
public class SaveGameManager : MonoBehaviour
{
	...
	public SaveGameManager.AssetReference GetAssetId(UnityEngine.Object referencedObject)
	{
		...
		if (!this.assetReferences.TryGetValue(type, out index))
		{
			...
			foreach (UnityEngine.Object @object in enumerable)
			{
				index[@object.name].Add(@object);
			}
		}
	...
	private void RecurseAddInactive(IEnumerable<Transform> items)
	{
		...
		{
			if (transform.GetComponent<UniqueIdentifier>() != null && !transform.gameObject.active)
			{
				SaveGameManager.GetId(transform.gameObject);
			}
			this.RecurseAddInactive(transform.Cast<Transform>());
		}
	}
	...
	private void Awake()
	{
		...
		{
			foreach (Action action in SaveGameManager._initActions)
			{
				action();
			}
