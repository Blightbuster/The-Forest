...
public class lb_BirdController : MonoBehaviour
{
	...
	private void checkHandConditions()
	{
		...
		if (LocalPlayer.GameObject)
		{
			...
			if (LocalPlayer.Rigidbody.velocity.magnitude < 0.2f && !this.breakBool && Scene.SceneTracker.allAttackers == 0 && !LocalPlayer.Animator.GetBool("bookHeld") && !LocalPlayer.FpCharacter.PushingSled)
			{
				...
				if (ForestVR.Enabled)
				{
					num4 = Vector3.Angle(-LocalPlayer.vrPlayerControl.LeftHand.transform.right, Vector3.up);
					num5 = Vector3.Distance(LocalPlayer.vrPlayerControl.LeftHand.transform.position, this.prevHandPos);
				}
				this.prevHandPos = LocalPlayer.vrPlayerControl.LeftHand.transform.position;
				if (this.repeatCount > num3 && UnityEngine.Random.value < num2 && num4 < 26.5f && num5 < 0.7f)
				{
					LocalPlayer.GameObject.SendMessage("doBirdOnHand");
					this.repeatCount = 0;
					this.breakBool = true;
					base.Invoke("resetBreakBool", 70f);
				}
			}
			else
			{
	...
	{
		while (Scene.HudGui.Loading._cam.activeSelf)
		{
			yield return null;
		}
		base.InvokeRepeating("checkHandConditions", 1f, 5f);
		if (BoltNetwork.isClient)
		{
			yield break;
		}
	...
	private void Update()
	{
		if (Scene.SceneTracker == null || Scene.HudGui.Loading._cam.activeSelf)
		{
			return;
		}
	...
	private void updateRemoveBirdTargets()
	{
		...
		else if (this.removeBirdIndex < this.myBirds.SafeCount<GameObject>() && this.myBirds[this.removeBirdIndex] != null && this.myBirds[this.removeBirdIndex].activeSelf && Scene.SceneTracker.GetClosestPlayerDistanceFromPos(this.myBirds[this.removeBirdIndex].transform.position) > this.unspawnDistance)
		{
			this.Unspawn(this.myBirds[this.removeBirdIndex]);
		}
		this.removeBirdIndex = this.myBirds.WrapIndex(++this.removeBirdIndex, 0);
