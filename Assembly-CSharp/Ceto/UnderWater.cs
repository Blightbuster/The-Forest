...
namespace Ceto
{
	...
	public class UnderWater : UnderWaterBase
	{
		...
		private void Start()
		{
			try
			{
				this.startBelowInScatterScale = this.belowInscatterModifier.scale;
		...
		private void Update()
		{
			try
			{
				...
				if (LocalPlayer.Buoyancy && Application.isPlaying)
				{
					float b = this.startBelowInScatterScale;
					if (LocalPlayer.Buoyancy.ignoreOceanTriggers)
					{
						b = 0f;
					}
					this.modBelowInscatterScale = Mathf.Lerp(this.modBelowInscatterScale, b, Time.deltaTime * 2f);
				}
				else
				{
					this.modBelowInscatterScale = this.belowInscatterModifier.scale;
				}
				Shader.SetGlobalFloat("Ceto_BelowInscatterScale", Mathf.Max(0.1f, this.modBelowInscatterScale));
		...
		public AbsorptionModifier subSurfaceScatterModifier = new AbsorptionModifier(10f, 1.5f, new Color32(220, 250, 180, byte.MaxValue));

		public InscatterModifier aboveInscatterModifier = new InscatterModifier(50f, 1f, new Color32(2, 25, 43, byte.MaxValue), INSCATTER_MODE.EXP);

		public InscatterModifier belowInscatterModifier = new InscatterModifier(60f, 1f, new Color32(7, 51, 77, byte.MaxValue), INSCATTER_MODE.EXP);
		...
		private float modBelowInscatterScale;

		private float startBelowInScatterScale;
