...
public class Octree
{
	...
	private void Clear(Octree.Node n)
	{
		...
		foreach (KeyValuePair<int, Transform> keyValuePair in n.Transforms)
		{
	...
	private void Purge(Octree.Node n)
	{
		...
		if (n.Children != null)
		{
			...
			if (n.Level > 2 && num2 <= 8 && num == 8)
			{
				...
				{
					this.Insert(n, transform, transform.GetInstanceID(), transform.position);
				}
			}
		}
	}
	...
	private void Insert(Octree.Node n, Transform t, int id, Vector3 p)
	{
		if (n.Children == null)
		{
			...
			this.Divide(n);
			foreach (Transform transform in n.Transforms.Values)
			{
				this.Insert(n, transform, transform.GetInstanceID(), transform.position);
			}
			n.Transforms.Clear();
