...
public class Buoyancy : MonoBehaviour
{
	...
	public float LastWaterEnterSpeed { get; private set; }

	private void Awake()
	{
		if (this.DisableIfClient && BoltNetwork.isClient)
		{
			base.enabled = false;
		}
		if (!this.SkipAwakeVoxelise && (this.Voxels == null || this.Voxels.Length <= 0))
		{
			Debug.LogWarning("Buoyant object not voxelized... Doing this at runtime can be slow!");
			this.Voxelize(base.transform, 0f);
		}
	}
	...
	{
		foreach (Vector3 vector in Buoyancy._directions)
		{
	...
	private void OnTriggerEnter(Collider other)
	{
		...
		if (other.CompareTag("Water"))
		{
			...
			if (!this.WaterTriggers.Contains(other))
			{
				...
				IsOcean component = other.GetComponent<IsOcean>();
				if (this.ignoreOceanTriggers)
				{
					this.IsOcean = false;
				}
				else if (component && this.isPlayer && !this.isPlayerNet && LocalPlayer.IsInCaves)
				{
					this.ignoreOceanTriggers = true;
					this.IsOcean = false;
				}
				else
				{
				...
				if (component2)
				{
					this.LastWaterEnterSpeed = component2.velocity.magnitude;
				}
			}
		}
	}
	...
	private void OnTriggerExit(Collider other)
	{
		...
		if (other.CompareTag("Water") && this.WaterTriggers.Contains(other))
		{
			...
			{
				if (!this.lastWaterCollider || this.lastWaterCollider.Equals(other))
				{
					this.lastWaterCollider = this.WaterTriggers.LastOrDefault<Collider>();
