...
namespace TheForest.World
{
	...
	public class DoorWeightOpener : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			if ((float.IsNaN(this._doorVelocity) || Mathf.Abs(this._doorVelocity) < 0.001f) && Mathf.Abs(this._weightVelocity.sqrMagnitude) < 0.001f)
			{
				this._doorVelocity = 0f;
				this._weightVelocity = Vector3.zero;
				base.enabled = false;
			}
		}
		...
		{
			this.PlayOpenEvent();
			this._doorVelocity = Mathf.Max(this._doorVelocity, 0.0015f);
			this._weight++;
		...
		{
			this._doorVelocity = Mathf.Min(this._doorVelocity, -0.0015f);
			this._weight--;
			this.RefreshDoorAndWeightTarget();
