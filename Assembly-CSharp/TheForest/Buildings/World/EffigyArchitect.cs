...
namespace TheForest.Buildings.World
{
	...
	public class EffigyArchitect : EntityBehaviour
	{
		private void Awake()
		{
		...
		private void Update()
		{
			if (!this._enableEffigy.lightBool || !this.CanLight)
			{
		...
		{
			if (this._enableEffigy)
			{
				this._enableEffigy.SendMessage("receiveLightFire");
			}
		}
		...
		private void UpdateCurrentPreviewItem(int itemId)
		{
			if (this._currentPreviewItemId != itemId)
			{
				...
				else if (itemId == -1)
				{
					...
					if (this.CanLight)
					{
						this._enableEffigy.SendMessage("GrabEnter");
					}
				}
			}
		}
