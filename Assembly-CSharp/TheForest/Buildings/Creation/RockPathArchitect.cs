...
{
	[AddComponentMenu("Buildings/Creation/Rock Path Architect")]
	...
	public class RockPathArchitect : WallArchitect
	{
		...
		protected override void ShowTempWall()
		{
			...
			{
				p2 = MathEx.TryAngleSnap(base.transform.position, this._multiPointsPositions[this._multiPointsPositions.Count - 1], this._multiPointsPositions[this._multiPointsPositions.Count - 2], PlayerPreferences.BuildingSnapAngle, PlayerPreferences.BuildingSnapRange, true);
		...
		protected override float MinLockLength
		{
			get
			{
				return (float)this._minRockCount * this._logWidth;
			}
		}
