...
namespace TheForest.Buildings.Creation
{
	...
	public class RoofArchitect : EntityBehaviour, IStructureSupport, IHoleStructure, ICoopStructure, IRoof, IEntityReplicationFilter
	{
		...
		protected void InitRowPointsBuffer(int rowCount, List<Vector3>[] buffer, out List<Vector3>[] outBuffer)
		{
		...
		{
			return Mathf.FloorToInt(Mathf.Abs(zPosition - minZ) / this.RowWidth);
		}

		private int GetUpperRowIndex(float zPosition, float minZ)
		{
			return Mathf.CeilToInt(Mathf.Abs(zPosition - minZ) / this.RowWidth);
		}
		...
		protected void CalcRowPointBufferForArray(Vector3[] localPoints, float minLocalZ, List<Vector3>[] rowPointsBuffer)
		{
			...
			for (int i = 1; i < localPoints.Length; i++)
			{
				...
				num2 += Mathf.CeilToInt((Mathf.Abs(vector.z) - Mathf.Abs(num5)) / rowWidth);
				item.z += num5;
				item.x += num8;
		...
		protected virtual void SpawnRoof()
		{
			...
			float num11 = Mathf.Clamp(this._roofHeight, 0f, (float)this._rowCount / 2f * (this.LogWidth / 2f));
			...
			for (int num28 = 0; num28 < this._rowCount; num28++)
			{
				...
				for (int num29 = 1; num29 < count3; num29 += 2)
				{
					...
					vector6.y = y + (1f - Mathf.InverseLerp(num12, num13, value5) * num16) * this._roofHeight;
					...
					if ((num33 > 0f && num34 > 0f) || (num33 < 0f && num34 < 0f))
					{
						if (num33 > 0f)
						{
							...
							this.SpawnChunk(a2 - vector7, vector7, roofRoot, logStackScaleRatio, ref num27);
						}
						else if (num33 < 0f)
						{
							Vector3 vector8 = Vector3.Lerp(vector6, vector4, Mathf.Abs(num33));
							Vector3 a3 = Vector3.Lerp(vector6, vector4, Mathf.Abs(num34));
							this.SpawnChunk(a3 - vector8, vector8, roofRoot, logStackScaleRatio, ref num27);
						}
					}
					else
					{
						...
						Vector3 a4 = vector6;
						this.SpawnChunk(a4 - vector9, vector9, roofRoot, logStackScaleRatio, ref num27);
		...
		protected void SpawnChunk(Vector3 chunk, Vector3 chunkStart, Transform currentLogStackTr, float logStackScaleRatio, ref float currentLogStackRemainingScale)
		{
			if (chunk.sqrMagnitude > 0f)
			{
				Quaternion rotation = Quaternion.LookRotation(chunk);
				float num = chunk.magnitude * this.RowChunkLengthRatio;
				int num2 = Mathf.CeilToInt(num / this._maxSegmentHorizontalLength);
				Vector3 one = Vector3.one;
				one.z = num / (float)num2 / this._logLength;
				Vector3 b = chunk / (float)num2;
		...
		public virtual float RowChunkLengthRatio
		{
			get
			{
				return 1f;
		...
		[SerializeThis]
		protected int _holesCount;

		[SerializeThis]
		protected float _roofHeight;

		protected Transform _roofRoot;
		...
		protected float _minEdgeLength;

		protected int _rowCount;

		protected List<Vector3>[] _rowPointsBuffer;

		protected List<Vector3>[] _holesRowPointsBuffer;

		protected Stack<Transform> _logPool;

		protected Stack<Transform> _newPool;

		protected RoofArchitect.LockModes _lockMode;
