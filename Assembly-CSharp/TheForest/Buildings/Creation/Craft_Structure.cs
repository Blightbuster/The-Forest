...
namespace TheForest.Buildings.Creation
{
	...
	public class Craft_Structure : EntityBehaviour
	{
		...
		public bool GetBuildInputDown()
		{
			if (!ForestVR.Enabled)
			{
				return TheForest.Utils.Input.GetButtonDown("Build");
			}
			if (TheForest.Utils.Input.GetAxis("Build") > 0.5f && !this._buildInputDown)
			{
				this._buildInputDown = true;
				return true;
			}
			if (TheForest.Utils.Input.GetAxis("Build") < 0.5f && this._buildInputDown)
			{
				this._buildInputDown = false;
				return false;
			}
			return false;
		}
