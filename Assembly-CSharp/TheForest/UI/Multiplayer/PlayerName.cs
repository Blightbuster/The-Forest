...
namespace TheForest.UI.Multiplayer
{
	public class PlayerName : MonoBehaviour
	{
		...
		private void LateUpdate()
		{
			if (LocalPlayer.MainCam)
			{
				...
				else
				{
					...
					{
						num2 = Mathf.Lerp(this._nearRangeTextSize, this._farRangeTextSize, (num - this._nearRange) / (this._farRange - this._nearRange));
					}
					else
					{
						num2 = this._nearRangeTextSize;
					}
					...
					if (ForestVR.Enabled)
					{
						...
						this._vrOverlay.transform.localScale = new Vector3(num3, num3, num3);
						this._vrOverlay.transform.LookAt(LocalPlayer.MainCamTr.position, LocalPlayer.Transform.rotation * Vector3.up);
						this._VrTextMesh.text = this._overlay._name.text;
					}
			...
			if (ForestVR.Enabled)
			{
				this._vrOverlay.SetActive(this._overlay.gameObject.activeSelf);
			}
		}
		...
		public GameObject _vrOverlay;

		public TextMesh _VrTextMesh;
