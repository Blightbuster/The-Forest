...
namespace TheForest.Utils
{
	...
	public class TreeLodGrid : MonoBehaviour
	{
		...
		private void InitGrid()
		{
			...
			foreach (LOD_Trees lod_Trees in array)
			{
		...
		private float CalcCellLodRatio(int x, int y)
		{
			...
			float value = this._minOutput + (float)num3 * this._openDirsOutputContribution / (float)num;
			return Mathf.Clamp(value, this._minOutput, this._maxOutput);
		}
		...
		public bool _showDensityGrid;

		public bool _showDistanceCircles;
