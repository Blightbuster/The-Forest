...
namespace TheForest.Items.World
{
	...
	public class PickUp : MonoBehaviour
	{
		protected virtual void Awake()
		{
			...
			this.SetActionTextOverrides();
		}

		private void SetActionTextOverrides()
		{
			PickUp.SetActionTextOverrides(this._sheen);
			PickUp.SetActionTextOverrides(this._myPickUp);
		}

		private static void SetActionTextOverrides(GameObject target)
		{
			if (target == null)
			{
				return;
			}
			SheenBillboard[] componentsInChildren = target.GetComponentsInChildren<SheenBillboard>(true);
			foreach (SheenBillboard sheenBillboard in componentsInChildren)
			{
				sheenBillboard.ActionTextOverride = "TAKE";
			}
		}
		...
		private void Update()
		{
			...
			if (this._enableTime < Time.time && ((this._inputMethod == PickUp.InputMethods.ButtonDown && this.GetTakeInputDown()) || (this._inputMethod == PickUp.InputMethods.ButtonUp && TheForest.Utils.Input.GetButtonUp("Take")) || (this._inputMethod == PickUp.InputMethods.ButtonHold && TheForest.Utils.Input.GetButtonAfterDelay("Take", 0.5f, false)) || (this._inputMethod == PickUp.InputMethods.ButtonPress && TheForest.Utils.Input.GetButtonPress("Take"))))
			{
				if (this._useSkinAnimalRoutine)
				{
		...
		public bool GetTakeInputDown()
		{
			if (!ForestVR.Enabled)
			{
				return TheForest.Utils.Input.GetButtonDown("Take");
			}
			if (TheForest.Utils.Input.GetAxis("Take") > 0.5f && !this._takeInputDown)
			{
				this._takeInputDown = true;
				return true;
			}
			if (TheForest.Utils.Input.GetAxis("Take") < 0.5f && this._takeInputDown)
			{
				this._takeInputDown = false;
				return false;
			}
			return false;
		}
