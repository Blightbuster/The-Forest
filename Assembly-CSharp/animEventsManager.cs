...
public class animEventsManager : EntityEventListener
{
	...
	{
		if (ForestVR.Enabled && !this.Remote)
		{
			if (!LocalPlayer.Inventory.IsSlotEmpty(Item.EquipmentSlot.RightHand))
			{
				Vector3 vector = this._prevWepPos - LocalPlayer.vrPlayerControl.VROffsetTransform.InverseTransformPoint(this._weaponVelocityTr.position);
				Vector3 from = this._weaponVelocityTr.position - this._prevWorldWepPos;
				Vector3 vector2 = this._prevWristPos - LocalPlayer.vrPlayerControl.VROffsetTransform.InverseTransformPoint(LocalPlayer.vrPlayerControl.RightIkOffset.position);
				float num = vector.magnitude * 100f;
				float num2 = vector2.magnitude * 100f;
				this._prevWepPos = LocalPlayer.vrPlayerControl.VROffsetTransform.InverseTransformPoint(this._weaponVelocityTr.position);
				this.fixedUpdateCount = 0;
				this._prevWristPos = LocalPlayer.vrPlayerControl.VROffsetTransform.InverseTransformPoint(LocalPlayer.vrPlayerControl.RightIkOffset.position);
				this._prevWorldWepPos = this._weaponVelocityTr.position;
				this._startSwingPositions[this._swingPosCounter] = this._prevWepPos;
				this._swingPosCounter++;
				if (this._swingPosCounter > 9)
				{
					this._swingPosCounter = 0;
				}
				float num3 = 0f;
				float num4 = this._vrEnableWeaponVelocity;
				float num5 = 0.5f;
				float num6 = 45f;
				float num7 = this.minWristVelocity;
				num7 *= 0.5f;
				if (this.animator.GetBool("axeHeld"))
				{
					num3 = Vector3.Angle(from, this._weaponVelocityTr.forward);
				}
				else if (this.animator.GetBool("spearHeld"))
				{
					num4 *= 0.23f;
					num5 = 0.3f;
					num3 = ((!LocalPlayer.vrPlayerControl.RightHand.GetStandardInteractionButton()) ? Vector3.Angle(from, this._weaponVelocityTr.up) : Vector3.Angle(from, -this._weaponVelocityTr.up));
					num6 = 60f;
				}
				if (num > num4)
				{
					float num8 = Vector3.Distance(this.arcStartPosition, LocalPlayer.vrPlayerControl.VROffsetTransform.InverseTransformPoint(this._weaponVelocityTr.position));
					if (num8 > this._minArcDistance * num5 && num3 < num6 && num2 > num7)
					{
				...
				{
					this.arcStartPosition = LocalPlayer.vrPlayerControl.VROffsetTransform.InverseTransformPoint(this._weaponVelocityTr.position);
				}
			}
			else
			{
	...
	private void throwHairSpray()
	{
		if (this.Remote)
		{
			if (!this.hairSprayHeldGo.activeSelf)
			{
				return;
			}
		}
		...
		{
			if (!LocalPlayer.Inventory.Owns(this._hairSprayId, true))
			{
				return;
			}
		...
		if (this.Remote)
		{
			position = this.rightArmSplashPos.position;
		}
		else
		{
			position = LocalPlayer.ScriptSetup.rightHandWrist.position;
		}
		GameObject gameObject = UnityEngine.Object.Instantiate<GameObject>(this.hairSprayEmptyGo1, position, Quaternion.identity);
		...
		if (this.Remote)
		{
			vector = base.transform.forward * 2000f * (0.016666f / Time.fixedDeltaTime);
		}
		else
		{
			vector = LocalPlayer.MainCamTr.forward * 2000f * (0.016666f / Time.fixedDeltaTime);
		}
		component.AddForce(vector, ForceMode.Force);
		component.AddTorque(vector, ForceMode.Force);
		if (!this.Remote)
		{
			LocalPlayer.Inventory.RemoveItem(this._hairSprayId, 1, false, true);
		}
	}
	...
	public void throwSpear()
	{
		...
		if (!ForestVR.Enabled && (LocalPlayer.AnimControl.currLayerState1.tagHash == LocalPlayer.AnimControl.knockBackHash || LocalPlayer.AnimControl.nextLayerState1.tagHash == LocalPlayer.AnimControl.knockBackHash || LocalPlayer.AnimControl.fullBodyState2.tagHash == LocalPlayer.AnimControl.knockBackHash || LocalPlayer.AnimControl.nextFullBodyState2.tagHash == LocalPlayer.AnimControl.knockBackHash))
		{
			return;
		}
	...
	public GameObject hairSprayEmptyGo1;

	public GameObject hairSprayHeldGo;
	...
	private Vector3[] _startSwingPositions = new Vector3[10];

	private float _vrWeaponAttackWindow = 0.4f;

	public float _vrEnableWeaponVelocity = 11f;

	public float minWristVelocity = 5f;

	public float _minArcDistance = 0.25f;
