...
public class PlayerSfx : EntityEventListener<IPlayerState>
{
	...
	public void SetHangGliderLoop(bool onOff)
	{
		PLAYBACK_STATE state;
		UnityUtil.ERRCHECK(this.GliderLoopInstance.getPlaybackState(out state));
		if (onOff)
		{
			UnityUtil.ERRCHECK(this.GliderLoopInstance.set3DAttributes(UnityUtil.to3DAttributes(base.gameObject, null)));
			UnityUtil.ERRCHECK(this.GliderLoopSpeedParameter.setValue(LocalPlayer.Rigidbody.velocity.sqrMagnitude / 2500f));
			this.SyncLoopingEvent(this.GliderLoopEvent, base.transform.position);
			if (!state.isPlaying())
			{
				UnityUtil.ERRCHECK(this.GliderLoopInstance.start());
			}
		}
		else
		{
			...
			if (state.isPlaying())
			{
				UnityUtil.ERRCHECK(this.GliderLoopInstance.stop(STOP_MODE.ALLOWFADEOUT));
			}
		}
	}
	...
	private EventInstance GliderLoopInstance;

	private ParameterInstance GliderLoopSpeedParameter;
