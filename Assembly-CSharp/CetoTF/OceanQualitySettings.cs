...
namespace CetoTF
{
	public class OceanQualitySettings : MonoBehaviour
	{
		...
		{
			if (Ocean.Instance == null || LocalPlayer.MainCam == null)
			{
				return;
			}
			if (this.usingSharedDepthGrab)
			{
				...
				DepthBufferGrabCommand.AddBinding(LocalPlayer.MainCam, Ocean.DEPTH_GRAB_TEXTURE_NAME);
			}
			else
			{
				Ocean.Instance.UnderWater.DisableCopyDepthCmd = false;
				DepthBufferGrabCommand.RemoveBinding(LocalPlayer.MainCam, Ocean.DEPTH_GRAB_TEXTURE_NAME);
			}
