...
public class PlayerStats : MonoBehaviour, IBurnable
{
	...
	private IEnumerator OnDeserialized()
	{
		...
		this.ColdArmor = Mathf.Clamp(this.ColdArmor, 0f, 1f);
		yield return null;
		...
		while (LocalPlayer.CurrentView == PlayerInventory.PlayerViews.Loading)
		{
			yield return null;
		}
		yield return null;
	...
	private void Awake()
	{
		...
		if (!LevelSerializer.IsDeserializing && !PlayerSpawn.LoadingSavedCharacter)
		{
			if (BoltNetwork.isRunning)
			{
				...
				if (this.PlayerVariations.Variations[this.PlayerVariation].Hair.Length > 0)
				{
	...
	private void Start()
	{
		...
		if (TheForest.Utils.Scene.Cams.SleepCam.activeSelf)
		{
			...
			UnityEngine.Debug.Log("disable sleep cam");
		}
		else
		{
	...
	private IEnumerator OnSaveSlotSelectedRoutine()
	{
		...
		if (!LocalPlayer.IsInOverlookArea)
		{
			if (PlayerPreferences.MemorySafeSaveMode)
			{
				...
				yield return null;
				yield return ResourcesHelper.UnloadUnusedAssets();
				yield return null;
	...
	public void CheckStats()
	{
		if (this.IsHealthInGreyZone)
		{
			if (!this.dyingVisionEnabled)
			{
				if (!ForestVR.Enabled)
				{
					this.DyingVision.enabled = true;
				}
				this.dyingVisionEnabled = true;
		...
		else if (this.dyingVisionEnabled)
		{
			this.DyingVision.enabled = false;
			Mood.PlayerLowHealth(false);
			this.dyingVisionEnabled = false;
		}
	}
	...
	private void ToggleArmorPiece(PlayerStats.ArmorTypes modelType, Material mat, int index, bool onoff)
	{
		if (modelType != PlayerStats.ArmorTypes.None)
		{
			...
			if (armorPiece)
			{
				...
				if (onoff && mat)
				{
					Renderer component = armorPiece.GetComponent<Renderer>();
					if (component)
					{
	...
	private void DeathInWater(int damage = 0)
	{
		...
		if (ForestVR.Enabled)
		{
			base.Invoke("EnableGhostMode", 0.1f);
		}
		if (BoltNetwork.isRunning)
		{
			this.Dead = true;
	...
	private void EnableGhostMode()
	{
		...
		LocalPlayer.vrPlayerControl.gameObject.SendMessage("setVrStandPos1", LocalPlayer.vrAdapter.behindCamPos, SendMessageOptions.DontRequireReceiver);
		LocalPlayer.vrPlayerControl.ToggleVrControllers(true);
	}
	...
	public IEnumerator dragAwayCutScene()
	{
		...
		{
			LocalPlayer.vrPlayerControl.useGhostMode = true;
			LocalPlayer.vrPlayerControl.gameObject.SendMessage("setVrStandPos1", LocalPlayer.vrAdapter.dragAwayCamPos);
		}
		yield return YieldPresets.WaitForFixedUpdate;
		...
		TheForest.Utils.Scene.Cams.SleepCam.SetActive(false);
		yield return YieldPresets.WaitNineSeconds;
		yield return YieldPresets.WaitPointFiveSeconds;
		TheForest.Utils.Scene.Cams.SleepCam.SetActive(true);
	...
	public IEnumerator hangingInCaveCutScene()
	{
		...
		if (ForestVR.Enabled)
		{
			LocalPlayer.ScriptSetup.weaponCube.transform.parent.localScale = new Vector3(LocalPlayer.ScriptSetup.weaponCube.transform.parent.localScale.x, LocalPlayer.ScriptSetup.weaponCube.transform.parent.localScale.y, 2.3f);
			LocalPlayer.vrPlayerControl.useGhostMode = true;
			LocalPlayer.vrPlayerControl.gameObject.SendMessage("setVrStandPos1", TheForest.Utils.Scene.SceneTracker.HangingCaveCamTransform);
		...
		while (!lighterSkip)
		{
			if (LocalPlayer.Inventory.HasInSlot(Item.EquipmentSlot.RightHand, LocalPlayer.AnimControl._planeAxeId))
			{
				lighterSkip = true;
			}
		...
		if (ForestVR.Enabled)
		{
			LocalPlayer.ScriptSetup.weaponCube.GetComponent<weaponInfo>().enabled = true;
			LocalPlayer.ScriptSetup.weaponCube.transform.parent.localScale = new Vector3(LocalPlayer.ScriptSetup.weaponCube.transform.parent.localScale.x, LocalPlayer.ScriptSetup.weaponCube.transform.parent.localScale.y, 2.3f);
		}
	...
	private IEnumerator releaseFromHanging()
	{
		...
		if (ForestVR.Enabled)
		{
			LocalPlayer.ScriptSetup.weaponCube.GetComponent<weaponInfo>().enabled = false;
		}
		LocalPlayer.CamRotator.rotationRange = new Vector2(0f, 0f);
		LocalPlayer.MainRotator.rotationRange = new Vector2(0f, 0f);
		LocalPlayer.FpCharacter.enabled = false;
		...
		{
			LocalPlayer.vrPlayerControl.useGhostMode = false;
			TheForest.Utils.Scene.HudGui.GuiCam.SetActive(true);
			LocalPlayer.vrPlayerControl.ToggleVrControllers(false);
		}
	...
	public IEnumerator EndgameWakeUp()
	{
		...
		{
			LocalPlayer.vrPlayerControl.useGhostMode = true;
			LocalPlayer.vrPlayerControl.gameObject.SendMessage("useSteppedGhostMode", SendMessageOptions.DontRequireReceiver);
		}
	...
	private IEnumerator forcePlayerPosToMarker(Transform m)
	{
		...
		int terrainMask = 67108864;
		for (;;)
		{
			AnimatorStateInfo p = LocalPlayer.Animator.GetCurrentAnimatorStateInfo(2);
			yield return new WaitForEndOfFrame();
			RaycastHit tHit;
			if (Physics.Raycast(LocalPlayer.ScriptSetup.hipsJnt.position + Vector3.up * 2f, Vector3.down, out tHit, 10f, terrainMask, QueryTriggerInteraction.Ignore))
			{
				float num = LocalPlayer.ScriptSetup.hipsJnt.position.y - 0.33f - tHit.point.y;
				LocalPlayer.PlayerBase.transform.position = new Vector3(LocalPlayer.PlayerBase.transform.position.x, LocalPlayer.PlayerBase.transform.position.y - num, LocalPlayer.PlayerBase.transform.position.z);
				m2.transform.position = new Vector3(m2.transform.position.x, m2.transform.position.y - num, m2.transform.position.z);
				m3.transform.position = new Vector3(m3.transform.position.x, m3.transform.position.y - num, m3.transform.position.z);
			}
	...
	private IEnumerator resetPlayerFromHeal()
	{
		...
		if (ForestVR.Enabled)
		{
			LocalPlayer.vrPlayerControl.useGhostMode = false;
			LocalPlayer.vrPlayerControl.ToggleVrControllers(false);
			LocalPlayer.vrPlayerControl.ShowRespawnPrompt(false);
		}
		LocalPlayer.Inventory.enabled = true;
		this.hitReaction.disableControllerFreeze();
		if (LocalPlayer.Inventory.CurrentView == PlayerInventory.PlayerViews.World)
	...
	public void playerLoadedFromRespawn()
	{
		...
		base.StartCoroutine(this.forceBlackScreen(2f));
	}
	...
	private bool dyingVisionEnabled;
