...
public class mutantController : MonoBehaviour, IMutantController
{
	...
	private void Start()
	{
		...
		{
			base.InvokeRepeating("setupWormSpawn", 500f, 500f);
		}
		base.Invoke("setupSpawnDays", 10f);
	}

	private void setupSpawnDays()
	{
		this.nextSpawnedWormDay = Clock.Day + UnityEngine.Random.Range(5, 15);
		this.nextMeganSpawnDay = Clock.Day + UnityEngine.Random.Range(2, 6);
	}
	...
	private void setupWormSpawn()
	{
		if (Cheats.NoEnemies || GameSetup.IsPeacefulMode)
		{
			return;
		}
		if (LocalPlayer.SavedData == null || this.spawnedWormSpawnerGo != null || LocalPlayer.IsInCaves || LocalPlayer.IsInEndgame || LocalPlayer.IsInClosedArea)
		{
			return;
		}
		if (Clock.Day < 40 || Clock.Day < this.nextSpawnedWormDay)
		{
			return;
		}
		...
		Transform transform = Scene.SceneTracker.allPlayers[UnityEngine.Random.Range(0, Scene.SceneTracker.allPlayers.Count)].transform;
		if (transform == null)
		{
			return;
		}
		if (Scene.SceneTracker.allPlayersInCave.Contains(transform.gameObject))
		{
			return;
		}
		Vector3 vector = transform.position + transform.forward * -65f;
		vector.y = Terrain.activeTerrain.SampleHeight(vector) + Terrain.activeTerrain.transform.position.y;
		Vector3 zero = Vector3.zero;
		if (AstarPath.active != null)
		{
			GraphNode node = AstarPath.active.GetNearest(vector, NNConstraint.Default).node;
			if (node == null)
			{
				return;
			}
			...
			zero = new Vector3((float)(node.position[0] / 1000), (float)(node.position[1] / 1000), (float)(node.position[2] / 1000));
		}
		this.spawnedWormSpawnerGo = UnityEngine.Object.Instantiate<GameObject>(this.wormSpawnerPrefab, zero, Quaternion.identity);
		this.nextSpawnedWormDay = Clock.Day + UnityEngine.Random.Range(5, 15);
		Debug.Log("successfully spawned a worm creature!!");
	}
	...
	{
		if (LocalPlayer.SavedData != null && LocalPlayer.SavedData.ExitedEndgame && this.numActiveGirlSpawns < 1 && UnityEngine.Random.value < 0.15f && Clock.Day >= this.nextMeganSpawnDay)
		{
			this.nextMeganSpawnDay = Clock.Day + 3;
			spawn.amount_girl = 1;
			spawn.creepySpawner = true;
			this.numActiveGirlSpawns++;
	...
	public GameObject wormSpawnerPrefab;

	private GameObject spawnedWormSpawnerGo;

	public int nextSpawnedWormDay;
	...
	public int numActiveGirlSpawns;
	...
	public List<GameObject> activeWorms = new List<GameObject>();
	...
	private int nextMeganSpawnDay;

	private int currMeganSpawnDay;
