...
public class TheForestAtmosphere : MonoBehaviour
{
	...
	private void Update()
	{
		...
		else
		{
			...
			bool flag3 = !LocalPlayer.Rigidbody || LocalPlayer.Rigidbody.velocity.sqrMagnitude > 0.5f || ChatBox.IsChatOpen || LocalPlayer.Inventory.CurrentView == PlayerInventory.PlayerViews.Book;
			...
			else if (this.CatchUpTimeOfDay)
			{
				...
				float num9 = MathEx.Easing.EaseInOutQuad(this.DelayedTimeOfDayAlpha, 0f, 1f, 1f);
				if (num9 >= 1f)
				{
					num9 = 1f;
					this.CatchUpTimeOfDay = false;
				}
				this.DelayedTimeOfDay = this.LerpToTimeOfDay(this.MovementStartTimeOfDay, num8, num9);
				this.Sun.transform.rotation = this.SunRotationAtTimeOfDay(this.DelayedTimeOfDay);
			}
			else if (flag3 || flag2)
			{
				...
				this.DelayedTimeOfDay = this.LerpToTimeOfDay(this.MovementStartTimeOfDay, num8, alpha);
				this.Sun.transform.rotation = this.SunRotationAtTimeOfDay(this.DelayedTimeOfDay);
				if (flag2)
				{
			...
			else
			{
				this.DelayedTimeOfDayAlpha = -1f;
				float num10;
				if (this.DelayedTimeOfDay <= num8)
				{
					num10 = num8 - this.DelayedTimeOfDay;
				}
				else
				{
					num10 = (num8 + 360f - this.DelayedTimeOfDay) % 360f;
				}
				if (num10 > 5f)
				{
					this.CatchUpTimeOfDay = true;
		...
		if (Application.isPlaying)
		{
			...
			{
				QualitySettings.shadowDistance = (float)num11;
			}
		}
		if (this.isDaytime())
		{
