...
namespace UnityEngine.PostProcessing
{
	...
	public sealed class TaaComponent : PostProcessingComponentRenderTexture<AntialiasingModel>
	{
		...
		public void SetProjectionMatrix(Func<Vector2, Matrix4x4> jitteredFunc)
		{
			...
			this.context.camera.useJitteredProjectionMatrixForTransparentRendering = true;
		...
		public void Render(RenderTexture source, RenderTexture destination)
		{
			...
			{
				num = Scene.SceneTracker.taaMotionClamp;
			}
			if (Application.isPlaying)
			{
				material.SetVector(TaaComponent.Uniforms._SharpenParameters, new Vector4(taaSettings.sharpen * num, 0f, 0f, 0f));
			}
			else
			{
			...
			if (Application.isPlaying)
			{
				material.SetVector(TaaComponent.Uniforms._FinalBlendParameters, new Vector4(taaSettings.stationaryBlending, taaSettings.motionBlending * num, 6000f, 0f));
			}
			else
			{
