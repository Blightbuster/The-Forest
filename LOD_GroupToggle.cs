...
public class LOD_GroupToggle : MonoBehaviour, IThreadSafeTask
{
	...
	public class LodLevel
	{
		...
		public void RefreshVisibility(bool newVisibility, bool forward)
		{
			...
			{
				this.Renderers[i].enabled = newVisibility;
				if (newVisibility && forward)
				{
					this.Renderers[i].SendMessage("ResetStippling", SendMessageOptions.DontRequireReceiver);
				}
				else if (!newVisibility && !forward)
				{
					this.Renderers[i].SendMessage("StippleOut", SendMessageOptions.DontRequireReceiver);
				}
			}
						...
						else
						{
							...
							else
							{
								...
								else
								{
									...
									{
										particleSystem.enableEmission = newVisibility;
									}
									else
									{
