...
namespace Serialization
{
	...
	public static class UnitySerializer
	{
		...
		static UnitySerializer()
		{
			...
			ushort num = 60000;
			foreach (string typeName in PreWarm.PrewarmTypes)
			{
				...
				{
					Dictionary<Type, ushort> prewarmedTypes = UnitySerializer.PrewarmedTypes;
					Type key = type;
					ushort num2 = num;
					num = num2 + 1;
					prewarmedTypes[key] = num2;
					UnitySerializer.PrewarmLookup.Add(type);
				}
			}
			num = 50000;
			foreach (string text in PreWarm.PrewarmNames)
			{
				Dictionary<string, ushort> prewarmedNames = UnitySerializer.PrewarmedNames;
				string key2 = text;
				ushort num3 = num;
				num = num3 + 1;
		...
		{
			foreach (IDeserialized deserialized in process.deserializedObjects)
			{
		...
		{
			foreach (IDeserialized deserialized in UnitySerializer.DeserializedObject)
			{
				try
				{
					deserialized.Deserialized();
		...
		public static void RunDeferredActions(int count = 1, bool clear = true)
		{
			...
			lock (fixupFunctions)
			{
				...
				{
					foreach (UnitySerializer.DeferredSetter deferredSetter in from f in UnitySerializer.FixupFunctions
					where f.enabled
					select f)
					{
						try
						{
							deferredSetter.deferredRetrievalFunction(deferredSetter.parameters);
					...
					foreach (Action action in UnitySerializer.FinalDeserialization)
					{
						action();
					}
				}
		...
		public static void RunDeferredActions(UnitySerializer.FinalProcess process, int count = 1, bool clear = true)
		{
			...
			lock (fixupFunctions)
			{
				...
				for (int i = 0; i < count; i++)
				{
					foreach (UnitySerializer.DeferredSetter deferredSetter in from f in process.deferredSetters
					where f.enabled
					select f)
					{
						try
					...
					foreach (Action action in process.deferredActions)
					{
						action();
					}
				}
			...
			using (new UnitySerializer.TypePusher(type))
			{
				...
				else
				{
					...
					if (!first)
					{
						...
						{
							foreach (KeyValuePair<Type, ISerializeObject> keyValuePair in UnitySerializer.SubTypeSerializers)
							{
								if (keyValuePair.Key.IsAssignableFrom(type) && (!keyValuePair.Value.GetType().IsDefined(typeof(OnlyInterfaces), false) || type.IsInterface))
								{
									value3 = keyValuePair.Value;
									break;
								}
							}
				...
				else
				{
					...
					else
					{
						...
						if (num2 != -1 && entry.Value == null && flag)
						{
							try
							{
								object result2 = UnitySerializer._loadedObjects[num2];
						...
						if (entry.Value == null)
						{
							if (UnitySerializer.Serializers.ContainsKey(storedType))
							{
								...
								object obj2 = serializeObject.Deserialize(data, entry.Value);
								storage.EndReadProperty();
								storage.EndReadObject();
								UnitySerializer._loadedObjects[num] = obj2;
								storage.FinishDeserializing(entry);
								return obj2;
							}
							...
							if (!UnitySerializer.cachedSerializers.TryGetValue(storedType, out serializeObject2))
							{
								...
								foreach (KeyValuePair<Type, ISerializeObject> keyValuePair in UnitySerializer.SubTypeSerializers)
								{
									if (keyValuePair.Key.IsAssignableFrom(storedType) && (!keyValuePair.Value.GetType().IsDefined(typeof(OnlyInterfaces), false) || storedType.IsInterface))
									{
										serializeObject2 = keyValuePair.Value;
										break;
									}
								}
							...
							if (serializeObject2 != null && (!(serializeObject2 is ISerializeObjectEx) || (serializeObject2 as ISerializeObjectEx).CanSerialize(storedType, entry.Value)))
							{
								...
								object obj3 = serializeObject2.Deserialize(data2, entry.Value);
								storage.EndReadProperty();
								storage.EndReadObject();
						...
						if (storedType.IsArray)
						{
							...
							if (flag2)
							{
								object obj4 = UnitySerializer.DeserializeMultiDimensionArray(storedType, storage, num);
								storage.EndReadObject();
							...
							{
								object obj5 = UnitySerializer.DeserializeArray(storedType, storage, count, num);
								storage.EndReadObject();
						...
						{
							object obj6 = entry.Value ?? UnitySerializer.CreateObject(storedType);
							if (storedType.IsValueType)
							{
								obj6 = RuntimeHelpers.GetObjectValue(obj6);
							}
							UnitySerializer._loadedObjects[num] = obj6;
							if (obj6 is IDictionary)
							{
								UnitySerializer.DeserializeDictionary(obj6 as IDictionary, storedType, storage);
							}
							if (obj6 is IList)
							{
								UnitySerializer.DeserializeList(obj6 as IList, storedType, storage);
							}
							object obj7 = UnitySerializer.DeserializeObjectAndProperties(obj6, storedType, storage);
							storage.EndReadObject();
							if (obj7 is IDeserialized)
							{
								UnitySerializer.DeserializedObject.Add(obj7 as IDeserialized);
							}
							if (obj6 is UnitySerializer.Nuller)
							{
								result = null;
							}
							else
		...
		private static readonly Dictionary<Type, IEnumerable<PropertyInfo>> PropertyLists = new Dictionary<Type, IEnumerable<PropertyInfo>>();

		
		private static readonly Dictionary<Type, IEnumerable<PropertyInfo>> ChecksumLists = new Dictionary<Type, IEnumerable<PropertyInfo>>();
		...
		private static readonly Dictionary<Type, ushort> PrewarmedTypes = new Dictionary<Type, ushort>();
		...
		private static readonly Dictionary<Type, Dictionary<string, UnitySerializer.EntryConfiguration>> StoredTypes = new Dictionary<Type, Dictionary<string, UnitySerializer.EntryConfiguration>>();
		...
		private static readonly Dictionary<Type, ISerializeObject> cachedSerializers = new Dictionary<Type, ISerializeObject>();
		...
		internal static List<UnitySerializer.DeferredSetter> FixupFunctions = new List<UnitySerializer.DeferredSetter>();

		
		internal static List<IDeserialized> DeserializedObject = new List<IDeserialized>();

		
		private static readonly Dictionary<Type, UnitySerializer.WriteAValue> Writers = new Dictionary<Type, UnitySerializer.WriteAValue>();

		
		public static readonly Dictionary<Type, UnitySerializer.ReadAValue> Readers = new Dictionary<Type, UnitySerializer.ReadAValue>();
