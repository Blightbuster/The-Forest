...
public static class Radical
{
	...
	{
		float currentVolume = (fromLevel == null) ? source.volume : fromLevel.Value;
		float t = 0f;
		while (t < 1f)
		{
	...
	{
		foreach (Transform transform in co.transform.GetComponentsInChildren<Transform>().Except(new Transform[]
		{
			co.transform
		}))
		{
	...
	private static void IndexScene()
	{
		...
		foreach (GameObject gameObject in (from GameObject g in UnityEngine.Object.FindObjectsOfType(typeof(GameObject))
		...
		select t.gameObject))
		{
			Radical._gameObjects[gameObject.name] = gameObject;
			Radical._fullPaths[gameObject.GetFullName().Substring(1)] = gameObject;
		}
	...
	public static Index<T, List<TR>> ToIndex<TSource, T, TR>(this IEnumerable<TSource> source, Func<TSource, T> keySelector, Func<TSource, TR> elementSelector) where T : class where TR : class
	{
		...
		{
			index[keySelector(tsource)].Add((elementSelector != null) ? elementSelector(tsource) : (tsource as TR));
		}
		return index;
	}
	...
	public static Index<T, List<TSource>> ToIndex<TSource, T>(this IEnumerable<TSource> source, Func<TSource, T> keySelector) where T : class
	{
		...
		foreach (TSource tsource in source)
		{
			index[keySelector(tsource)].Add(tsource);
		}
		return index;
	}
	...
	{
		foreach (Component obj2 in obj.GetComponents(t))
		{
	...
	public static Radical.PreferenceAccess Preferences = new Radical.PreferenceAccess();
	...
	public static Vector3 mergeMix = new Vector3(0f, 1f, 0f);
