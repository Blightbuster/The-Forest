...
public class animalSpawnFunctions : MonoBehaviour
{
	...
	private void updateSpawn()
	{
		if (this.fish)
		{
			...
			GameObject gameObject = SteamDSConfig.isDedicatedServer ? Scene.SceneTracker.GetClosestPlayerFromPos(this.thisTr.position) : Scene.SceneTracker.allPlayers[0];
			if (gameObject)
			{
				this.playerDist = Vector3.Distance(gameObject.transform.position, this.thisTr.position);
			}
		}
		if (this.turtle)
		{
			...
			GameObject closestPlayerFromPos = Scene.SceneTracker.GetClosestPlayerFromPos(this.thisTr.position);
			if (closestPlayerFromPos)
			{
				this.playerDist = Vector3.Distance(closestPlayerFromPos.transform.position, this.thisTr.position);
	...
	public void setSnowType(bool onoff)
	{
		...
		if (onoff)
		{
			base.transform.SendMessage("setOnSnow", true, SendMessageOptions.DontRequireReceiver);
		}
		else
		{
			base.transform.SendMessage("setOnSnow", false, SendMessageOptions.DontRequireReceiver);
		}
	}
