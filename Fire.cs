	...
	private void Update()
	{
		if (this.Lit)
		{
			...
			if (this.Player != null)
			{
				...
				if (!this.LeafDelay && this.Player.Owns(this._leafItemId, true))
				{
					this.AddLeavesIcon.SetActive(true);
					if (TheForest.Utils.Input.GetButtonDown("Take") && this.Player.RemoveItem(this._leafItemId, 1, false, true))
					{
						if (this.Player)
						{
