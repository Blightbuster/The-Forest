...
public class LinearEmitter : MonoBehaviour, FMOD_Listener.ILinearEmitter
{
	...
	private void StopAllSources()
	{
		...
		{
			foreach (LinearEmitter.Source source in this.sources)
			{
	...
	public float GetMaximumDistance()
	{
		...
		{
			num = Math.Max(num, LocalPlayer.Transform.InverseTransformPoint(base.transform.TransformPoint(position)).sqrMagnitude);
		}
	...
	public static List<LinearEmitter> sAwaitingBankLoad = new List<LinearEmitter>();
	...
	private static Texture2D occludedLineTexture = null;

	
	private static Texture2D occluderBorderTexture = null;

	
	private static Texture2D occluderCentreTexture = null;

	
	private static Texture2D sourceTexture = null;

	
	private static Texture2D pointTexture = null;

	
	private static Texture2D occludedPointTexture = null;
