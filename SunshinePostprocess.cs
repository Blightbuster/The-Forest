...
public class SunshinePostprocess : MonoBehaviour
{
	...
	private void OnDisable()
	{
		if (this.sunshine_cb != null)
		{
			...
			if (!Application.isPlaying && this.cam != null)
			{
				this.cam.RemoveCommandBuffer(CameraEvent.BeforeImageEffects, this.sunshine_cb);
			}
		}
	}
	...
	private void OnPreCull()
	{
		...
		Vector3 direction = this.cam.ViewportPointToRay(new Vector3(0f, 0f, 0f)).direction;
		Vector3 v = this.cam.ViewportPointToRay(new Vector3(1f, 0f, 0f)).direction - direction;
		Vector3 v2 = this.cam.ViewportPointToRay(new Vector3(0f, 1f, 0f)).direction - direction;
		...
		float y = 1f / (Mathf.Clamp01(value) * Sunshine.Instance.LightDistance / this.cam.farClipPlane);
		...
		int2 @int = new int2(this.cam.pixelWidth, this.cam.pixelHeight) / b;
		...
		if (this.sunshine_cb == null)
		{
			CommandBuffer[] commandBuffers = this.cam.GetCommandBuffers(CameraEvent.BeforeImageEffects);
		...
		if (this.sunshine_cb == null)
		{
			...
			this.cam.AddCommandBuffer(CameraEvent.BeforeImageEffects, this.sunshine_cb);
		}
		this.sunshine_cb.Clear();
		int nameID = Shader.PropertyToID("_SunshineScreenCopy");
		this.sunshine_cb.GetTemporaryRT(nameID, -1, -1, 0, FilterMode.Point, RenderTextureFormat.DefaultHDR, RenderTextureReadWrite.Default);
