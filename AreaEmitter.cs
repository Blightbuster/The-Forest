...
public class AreaEmitter : MonoBehaviour, FMOD_Listener.IAreaEmitter
{
	...
	private void Start()
	{
		...
		{
			this.eventDescription = FMOD_StudioSystem.instance.GetEventDescription(this.eventPath, true);
		}
	...
	private void StopAllSources()
	{
		if (this.sources.Count > 0)
		{
			foreach (AreaEmitter.Source source in this.sources)
			{
				if (source.eventInstance.isValid())
				{
					UnityUtil.ERRCHECK(source.eventInstance.stop(STOP_MODE.ALLOWFADEOUT));
					UnityUtil.ERRCHECK(source.eventInstance.release());
				}
	...
	public float GetMaximumDistance()
	{
		...
		for (int i = -1; i < this.voids.Count; i++)
		{
			foreach (Vector2 point in this.GetPolygon(i).points)
			{
				num = Math.Max(num, LocalPlayer.Transform.InverseTransformPoint(this.TransformPoint(point)).sqrMagnitude);
			}
		}
	...
	public void DrawDebug(Vector2 centre, float radius, float maximumDistance)
	{
		...
		foreach (AreaEmitter.Polygon polygon in this.voids)
		{
			this.DrawPolygon(polygon, centre, scale, sqrMaximumDistance, AreaEmitter.voidLineTexture, AreaEmitter.occludedVoidLineTexture, !flag);
		}
	...
	public class Polygon
	{
		...
		private void CalculateBounds()
		{
			...
			{
				this.bounds.xMin = Mathf.Min(this.bounds.xMin, vector.x);
				this.bounds.yMin = Mathf.Min(this.bounds.yMin, vector.y);
				this.bounds.xMax = Mathf.Max(this.bounds.xMax, vector.x);
				this.bounds.yMax = Mathf.Max(this.bounds.yMax, vector.y);
			}
		}
