...
public class mutantNetHitSetup : MonoBehaviour
{
	...
	private void disablePlayerCollision()
	{
		...
		{
			if (LocalPlayer.AnimControl.playerCollider.enabled && LocalPlayer.AnimControl.playerCollider.gameObject.activeSelf)
			{
				Physics.IgnoreCollision(this.controller, LocalPlayer.AnimControl.playerCollider, true);
			}
			if (LocalPlayer.AnimControl.playerHeadCollider.enabled && LocalPlayer.AnimControl.playerHeadCollider.gameObject.activeSelf)
			{
