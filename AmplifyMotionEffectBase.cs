...
public class AmplifyMotionEffectBase : MonoBehaviour
{
	...
	private void OnPostRender()
	{
		...
		float y = (this.DepthThreshold <= float.Epsilon) ? float.MaxValue : (1f / this.DepthThreshold);
		...
		if (flag2)
		{
			float num = (this.m_deltaTime < float.Epsilon) ? 0f : (this.MotionScale * this.CameraMotionMult * (1f / this.m_deltaTime));
